...
class YOLO:
    ...
    def _new(self, cfg: str, task=None, verbose=True):
        ...
        self.model = TASK_MAP[self.task][0](
            cfg_dict, verbose=verbose and RANK == -1)  # build model
        self.overrides['model'] = self.cfg

        # Below added to allow export from yamls
        # combine model and default args, preferring model args
        args = {**DEFAULT_CFG_DICT, **self.overrides}
        self.model.args = {k: v for k, v in args.items(
        ) if k in DEFAULT_CFG_KEYS}  # attach args to model
    ...
    def _check_is_pytorch_model(self):
        ...
        """
        pt_str = isinstance(self.model, (str, Path)) and Path(
            self.model).suffix == '.pt'
    ...
    def export(self, **kwargs):
        ...
        if overrides.get('imgsz') is None:
            # use trained imgsz unless custom value is passed
            overrides['imgsz'] = self.model.args['imgsz']
    ...
    def tune(self,
        ...
        tuner_callbacks = [WandbLoggerCallback(
            project='YOLOv8-tune')] if wandb else []
    ...
    def _reset_ckpt_args(args):
        ...
        include = {'imgsz', 'data', 'task',
                   'single_cls'}  # only remember these arguments when loading a PyTorch model
