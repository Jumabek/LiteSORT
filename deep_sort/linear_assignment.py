...
def min_cost_matching(
    ...
    cost_matrix[cost_matrix > max_distance] = max_distance + \
        1e-5  # dont think this has much effect
    ...
    for col, detection_idx in enumerate(detection_indices):
        if col not in col_indices:   # <-- CHANGE HERE
            unmatched_detections.append(detection_idx)
    for row, track_idx in enumerate(track_indices):
        if row not in row_indices:   # <-- CHANGE HERE
...
def gate_cost_matrix(
    ...
    for row, track_idx in enumerate(track_indices):
        ...
        if opt.MC:
            cost_matrix[row] = opt.MC_lambda * cost_matrix[row] + \
                (1 - opt.MC_lambda) * gating_distance
