...
run_tracker() {
    ...
    DIR_SAVE="results/${EXPERIMENT_NAME}/${DATASET}-train/woGating_${TRACKER_NAME}__input_${INPUT_RESOLUTION}__conf_${MIN_CONFIDENCE}/data/"
    if [ ! -d "${DIR_SAVE}" ]; then
        mkdir -p ${DIR_SAVE}
    fi
    ...
    case ${TRACKER_NAME} in
        "SORT")
            ...
            ;;
        "LiteSORT")
            ${BASE_CMD}  LiteSORT --dir_save ${DIR_SAVE} --input_resolution ${INPUT_RESOLUTION} --min_confidence ${MIN_CONFIDENCE} --appearance_feature_layer layer0
            ;;
        "DeepSORT")
