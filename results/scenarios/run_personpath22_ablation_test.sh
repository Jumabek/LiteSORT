...
SPLIT="test"
GPU_ID=3  # Set this to the GPU index you want to use (0, 1, 2, or 3)

# Base Command
BASE_CMD="python3 strong_sort.py ${DATASET} ${SPLIT}"

# Function to run LiteSORT tracker with layer specification and GPU assignment
run_tracker() {
...
for INPUT_RESOLUTION in 1280; do
    for MIN_CONFIDENCE in .25; do
        for LAYER in 0; do
            for MAX_COSINE_DISTANCE in 0.4; do  # Iterate over desired cosine distances
