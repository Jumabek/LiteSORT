...
DATASET="KITTI"
SPLIT="train"
CLASSES="0"  # Classes for vehicle detection

# Base Command
BASE_CMD="python3 strong_sort_fps.py ${DATASET} ${SPLIT}"
...
run_tracker() {
    ...
    DIR_SAVE="results/${EXPERIMENT_NAME}/${DATASET}-${SPLIT}_second_new_try/${TRACKER_NAME}__input_${INPUT_RESOLUTION}__conf_${MIN_CONFIDENCE}/data"
    if [ ! -d "${DIR_SAVE}" ]; then
        mkdir -p "${DIR_SAVE}"
    fi
