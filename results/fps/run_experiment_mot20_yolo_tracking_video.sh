...
MIN_CONFIDENCE=".25"
DATASET="MOT20"
PATH_TO_SEQ="/workspace/LiteSORT/datasets/MOT20/train/MOT20-01/MOT20-01.mp4"
TOTAL_FRAMES=429 # Adjust this according to your dataset
...
run_tracker() {
    ...
    echo "${TIME_OUTPUT}" | tee "${DIR_SAVE}/time_log_vid.txt"
    ...
    if (( $(echo "$REAL_TIME == 0" | bc -l) )); then
        ...
        echo "FPS: ${FPS}" | tee -a "${DIR_SAVE}/time_log_vid.txt"
    fi
...
# List of trackers
TRACKERS=('bytetrack' 'deepocsort' 'ocsort' 'botsort')
